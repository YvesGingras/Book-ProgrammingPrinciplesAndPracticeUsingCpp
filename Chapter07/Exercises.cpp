 ...
 *
 * This file has been created by a copy of the completed Drill.cpp one.
 *
 */

////Grammar to implementing the Drill specifications(Chapter 7)
/*
Calculation:
	Statement
	Print
	...
	int Calculate(TokenStream& tokenStream) {
		...
		while (cin)
			try
			{
				if (isFirstLoopDone)
					cout << prompt;
				Exercises::Token token = tokenStream.getToken();
				...
				cout << result << Exercises::Statement(tokenStream) << endl;
