...
//
// This is example code from Chapter 7.8.2 "Introducing names" of
...
{
	if (full) throw runtime_error("putback() into a full buffer");          //error("putback() into a full buffer");
...
Token Token_stream::get() // read characters from cin and compose a Token
{
	...
	switch (ch) {
		...
		default:
			...
			throw runtime_error("Bad token");		//error("Bad token");
	}
}
...
Target narrow_cast(Source src)
{
	...
	if ((Source)tgt != src)
		throw runtime_error("Invalid narrowing conversion");
